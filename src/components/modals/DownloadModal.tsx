...
export function DownloadModal(props: ModalDownloadProps) {
  ...
  if (!language || !resource)
    return (
      <Paragraph>A problem occured. Please close and try again.</Paragraph>
    );
  ...
  const back = (
    <BackButton data={{ onClick: () => setMember("init"), color: "black" }} />
  );
  ...
  return member === "init" ? (
    <>
      ...
      <Paragraph>
        Is your church a member of our church partnership program?
      </Paragraph>
    ...
    pic ? (
      ...
      <>
        ...
        <ChurchSelect
          value={pic}
          onChange={(pic) => onSelectPartner(pic)}
        ></ChurchSelect>
      </>
    )
  ) : member === "no" ? (
    ...
    <>
      ...
      <ButtonRow>
        <ButtonAnchor
          target="_blank"
          href="https://www.songsforsaplings.com/churches/"
        >
          LEARN MORE
        </ButtonAnchor>
        <Button secondary onClick={() => setMember("justDownload")}>
          SKIP TO DOWNLOAD
        </Button>
      </ButtonRow>
    </>
    ...
    <>
      ...
      <Paragraph>
        Our church partnership program offers free resources to churches around
        the world. If you are unsure if your church is a member, you can look up
        your church or join the program.
      </Paragraph>
      <ChurchSelect
        value={pic}
        onChange={(pic) => onSelectPartner(pic)}
