...
export function Modal(props: PopupProps) {
  ...
  const [zombieChildren, setZombieChildren] = useState<ReactNode | null>(
    children,
  );
  useEffect(() => {
    if (!children) {
  ...
  const intoBody = (
    <div
      className={`absolute top-0 left-0 w-full h-full transition-opacity bg-black bg-opacity-20 ${opacity}`}
      onClick={onClick}
    >
      <div className={`h-full transition-transform ${scale}`}>
        <ModalContainer onClose={onClose}>{dispChildren}</ModalContainer>
      </div>
    </div>
  );

  return dispChildren ? createPortal(intoBody, parent, "modal") : null;
}
