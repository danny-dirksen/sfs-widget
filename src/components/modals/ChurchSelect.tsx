...
export function ChurchSelect(props: ChurchSelectProps) {
  ...
  return (
    <>
      <div>
        <Input id='church-select' type='text' placeholder='Enter church name'
                value={search} onFocus={() => setFocused(true)} onBlur={() => setTimeout(() => setFocused(false), 100)}
        ...
        { focused && results && results.length > 0 ? (
          ...
          { results.map(({pic, name, url}) => {
            ...
            if (dispUrl.length > 30) dispUrl = dispUrl.substring(0, 37) + '...';
            return (
              <button key={pic} className='px-2 py-1 block w-full text-left hover:bg-sfs-accent text-black hover:text-white' onClick={onClick}>
                {name}
...
function useChurchSearch(query: string) {
  ...
  const results = useMemo<PartnerInfo[]>(() => {
    ...
    const res = (churches || []).map(church => ({
      church,
      key: (church.name+church.pic + church.url).replaceAll(/[^\w]/g, '').toLowerCase()
    }))
