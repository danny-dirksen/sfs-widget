...
export const Anchor = (props: { href: string; children: ReactNode }) => (
  <a
    href={props.href}
    className="text-sfs-accent underline font-bold"
    target="_blank"
  >
    {props.children}
  </a>
...
export const Header: FC = (props) => (
  <h1 {...props} className="text-sfs-accent font-bold text-xl">
    {props.children}
  </h1>
);

export const Paragraph: FC = (props) => <p {...props}>{props.children}</p>;

type InputProps = InputHTMLAttributes<HTMLInputElement>;

export const Input: React.FC<InputProps> = (props) => {
  const inputClass =
    "outline-none border border-2 border-black focus:border-sfs-accent px-2 py-1 w-full ";
  const { className, ...allOtherProps } = props;
  return <input {...allOtherProps} className={inputClass + className} />;
};
