...
export function DropdownOption(props: DropdownOptionProps) {
  ...
  const leftSide = href ? (
    <a onClick={onClick} href={href} className="w-full" target="_blank">
      {children}
    </a>
  ) : (
  ...
  const rightBtnClass =
    "size-7 p-1 widget:size-5 widget:p-0.5 hover:bg-black hover:bg-opacity-10";
  const rightSide =
    onClickInfo || onClickShare ? (
