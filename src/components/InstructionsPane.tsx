...
export function InstructionsPane(props: InstructionsPaneProps) {
  ...
  const channelName =
    content.channels.find((ch) => ch.channelId === channel)?.name || null;
  const languageName =
    content.languages.find((l) => l.languageId === language)?.autonym || null;
  ...
  return (
    <div
      className={
        "flex-shrink-0 hz:flex-[2] p-4 flex flex-col justify-center text-center gap-2 hz:gap-4 widget:gap-1 " +
