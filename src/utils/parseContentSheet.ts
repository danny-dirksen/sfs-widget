...
export async function parseContentSheet(
  ...
  for (let c = chanStartCol; c < sheet.columnCount; c++) {
    ...
    if (!name)
      return new Error("Channel " + channelId + " has id but no display name.");
  ...
  for (let start = headerRow + 1; start < sheet.rowCount; start++) {
    ...
    for (let r = start; r < end; r++) {
      ...
      for (let c = labels["resource.channels"]!; c < sheet.columnCount; c++) {
        ...
        if (!channelId) {
          return new Error(
            "Link " + url + ' is not attached to a resource id or "all"',
          );
        }
...
export function removeStubs(content: Content, enabledLinks: Link[]): Content {
  ...
  const enabledTranslations = resourceTranslations.filter((tran) =>
    enabledLinks.some(
      (link) =>
  ...
  const enabledChannels = channels.filter((chan) =>
  ...
  const enabledLanguages = languages.filter((lang) =>
    enabledTranslations.some((tran) => tran.languageId === lang.languageId),
  );
  const enabledResources = resources.filter((res) =>
    enabledTranslations.some((tran) => tran.resourceId === res.resourceId),
  );
