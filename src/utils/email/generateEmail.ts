 ...
 */
const downloadTemplate = fs.readFileSync(
  path.join(process.cwd(), "email-templates/download.html"),
  "utf8",
);
...
export function generateDownloadEmail(data: DownloadEmailData) {
  ...
  let shareBody = encodeURIComponent(
    `I found this great scripture-based kids' music that our family has really ` +
...
if (exampleEmail.search(/{{.*}}/) !== -1) {
  throw new Error(
    "Not all keys were replaced in the email template\n\n" +
