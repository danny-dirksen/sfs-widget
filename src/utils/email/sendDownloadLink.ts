...
mailchimp.setConfig({apiKey: env.MAILCHIMP_KEY, server: env.MAILCHIMP_PREFIX});
(async () => {
 ...
 */
export async function sendDownloadLink(emailData: DownloadEmailData): Promise<boolean> {
  ...
  const { msg, err } = await new Promise<{msg?: MessagesSendResult, err?: any}>((res) => {
      mg.messages.create(env.MAILGUN_DOMAIN, {
  ...
  const subscribed = await subscribe(emailData.email, emailData.firstName, emailData.lastName);
  if (!subscribed) {
 ...
 * @returns true if successful.
 */
async function subscribe(email: string, firstName: string, lastName: string): Promise<boolean> {
