...
export async function parseCptSheet(sheetsDoc: GoogleSpreadsheet): Promise<ContentProfileTable | Error> {
   ...
   */
  const findHeaders = (row: number, required: string[]):
  ...
  const l1Headers = findHeaders(l1HeaderRow, ['pic', 'name', 'url', 'emailAddress', 'emailSubject', 'languages']);
  ...
  for (let r = l2HeaderRow + 1; r < sheet.rowCount; r ++) {
    ...
    for (let c = l1Headers['languages']; c < sheet.columnCount; c ++) {
      ...
      if (!language) {
        return Error(`Row ${r} column ${c} is checked, but no language in that column.'`);
      }
