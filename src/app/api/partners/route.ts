...
import { getPartnerInfoList } from "@/repositories/getPersistentData";
...
export async function GET(): Promise<NextResponse<PartnerInfo[]>> {
  ...
  if (partners instanceof Error) return new NextResponse("Server error while fetching partners.", { status: 500 });
