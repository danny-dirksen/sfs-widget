  ...
  if (songs != null) {
    if (client.pageNum == 1) {
      ...
      supportedChannels.forEach(name => {
        ...
        let dropdownContents = `<img class="dropdown-logo" src="${path}" alt="${nameCaps}"></img>`;
        if (path == null) {
          dropdownContents = `<p class="dropdown-text">${nameCaps}</p>`;
        }
        let dropdown = `<div class="dropdown-option" onclick="sfsButtonClicked('select-channel', this)" data-channel="${name}">${dropdownContents}</div>`;
      ...
      supportedLanguages.forEach(language => {
        ...
        let dropdown = `<div class="dropdown-option" onclick="sfsButtonClicked('select-language', this)" data-language="${language}"><p class="dropdown-text">${autonym}</p></div>`;
